...
#from main import get_data_mode, get_dataset, create_model
#import torch

class VerificationNet:
    def __init__(
        self, force_learn: bool = False, file_name: str = "./models/verification_model_mono_binary_complete.weights.h5"
    ) -> None:
        """
    ...
    def check_class_coverage(
        self, data: np.ndarray, tolerance = 0.8
    ):
        """
    ...
    def check_predictability(
        self, data: np.ndarray, correct_labels: list = None, tolerance = 0.8
    ) -> tuple:
        """
        ...
        """
        color = (data.shape[3] == 3)
        
        predictions, beliefs = self.predict(data=data[:,:,:,0:1])
        if color:
            predictions1, beliefs1 = self.predict(data=data[:,:,:,1:2])
            predictions2, beliefs2 = self.predict(data=data[:,:,:,2:3])
            
            beliefs = beliefs * beliefs1 * beliefs2
            predictions = predictions * 100 + predictions1 * 10 + predictions2
...
if __name__ == "__main__":
    gen = StackedMNISTData(mode=DataMode.COLOR_BINARY_COMPLETE, default_batch_size=128)
    net = VerificationNet(force_learn=False, file_name="./models/verification_color_mono_binary_complete.weights.h5")
    net.train(generator=gen, epochs=15)
    
    
    # I have no data generator (VAE or whatever) here, so just use a sampled set
    img, labels = gen.get_random_batch(training=True, batch_size=2500)
    cov = net.check_class_coverage(data=img, tolerance=0.5)
