...
from utils import split_sequence, train_validate_test_split, CustomDataset
...
if __name__ == "__main__": 
    ...
    sel_model = "CNN" #['LSTM','CNN','FF']
    region = "NO5"
    savemodel = 1
    
    batch_size=64
    lr=0.0001
    no_labels = ["hour", "month", "NO1_temperature"]
    ...
    if sel_model == 'LSTM':
        ...
        model = CNN(input_size=input_lenght, output_size=1, in_channels=num_features, out_channels=3, kernel_size=3)
    elif sel_model == 'FF':
    ...
    mae = test_model(test, features, label, model, sel_model, input_lenght, output_lenght, num_plots=num_test_plot, scaler=scaler)
    print(f"mae: {mae}")
    
    if savemodel:
        print(f"{sel_model}_{region} model saved")
